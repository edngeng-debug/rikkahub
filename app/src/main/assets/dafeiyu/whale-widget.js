...
// 首次渲染直接用上次保存的角色（避免刷新瞬间先闪默认大肥鱼娘再切角色）。
...
function dshwCustSel(sel, opts) {
  ...
  btn.addEventListener('click', function (e) {
    ...
    // 主菜单内“任务结束音效”等下拉,避免列表盖住底部「大肥鱼记账」按钮
...
taskEndDrop = dshwCustSel(taskEndSel, {
  // 菜单态:下拉高度不越过底部「大肥鱼记账」按钮;窗口态(remind mask 存在)不做这个限制
...
// —— 隐藏菜单按钮:开启后右键/长按大肥鱼可唤出菜单(位置与按钮唤出一致) ——
...
menuHideToggle.title = '启用后隐藏挂件上的菜单按钮;电脑端右键、手机端长按大肥鱼可唤出菜单(位置不变)'
...
function setUsageNavBtn(inUsage) {
  try {
    usageRecBtn.textContent = inUsage ? '‹ 返回' : '- = 大肥鱼记账 = -'
...
function usageAlertBudgetEditor(key, onSave) {
  try {
    ...
    // 包装 renderBubblePv:列表/预览刷新后,大肥鱼预览里的占位符用实际数值实时替换
      ...
      postApiModels(collect(), function (d) {
        ...
        if (isNew) {
          ...
          showConfirm('✓ 保存成功：' + (nm || '新模型') + '\n已加入模型列表', function () {
            ...
            // 背后的「- = 大肥鱼记账 = -」列表同步刷新（模型行在静态区，需重建子界面）
            refreshModelList()
          }, '确认')
          return
        }
      ...
      fetch('/dsh-whale/role-delete.json', {
        ...
        .then(function (d) {
          if (d && d.ok && Array.isArray(d.roles)) {
            roleList = d.roles
            renderRolePanel()
            if (currentRole && currentRole.id === id) applyRole('default', '大肥鱼', IMG_URL)
            openResManager()
          }
        })
        .catch(function () {})
...
snapFlipRow.appendChild(snapMakeNum('F', '翻转线：距屏幕左边的位置，线左侧的大肥鱼会左右翻转'))
...
// —— 文本模块:完整悬浮编辑(内容/字体字号/样式颜色),实时刷新大肥鱼形预览 ——
...
bubbleBox.addEventListener('click', function (e) {
  ...
  // 点击泡泡 = 跳到下一项；已是最后一项则关闭（点大肥鱼才是从队列开头开始）
  bubbleNext()
})
...
//   关闭（默认）= 点大肥鱼时，正在看第 2 项及以后 → 回到第 1 项（旧行为）
//   开启       = 点大肥鱼 = 往后推进一项；已是最后一项 → 收起泡泡（与「点泡泡」一致），下次点按从第 1 项开始
...
function whaleClick() {
  try {
    if (!bubbleOn) return
    if (bubbleScene && (bubbleScene.kind === 'cost' || bubbleScene.kind === 'alert')) return // 消耗/预警提醒期间点大肥鱼不动作(点泡泡才关)
    if (!bubbleShown) {
      bubbleRoundOn = true
      bubbleSeqIdx = 0
      bubbleShowSeqNext()
...
function whaleSysTick() {
  ...
  try {
    ...
    // 存在尚未入队的正展示系统泡?仅在大肥鱼空闲且无我们自己队列项时取下一个
...
// 计算图像（大肥鱼 img 元素）的中心点：img 占挂件盒右下 59.45%（盒为正方形），
...
var scrollGapPx = 17
var menuBtnHide = false // 主菜单开关:隐藏挂件菜单按钮,改为右键大肥鱼唤出菜单
function saveConfig() {
  try {
...
function setUsageMode(v) {
  usageMode = 'ledger' // 大肥鱼记账为唯一记账方式(token 模式已下线)
  saveConfig()
  refresh(false)
}
...
function positionMenu() {
  try {
    ...
    // 菜单底边悬在大肥鱼素材顶部上方：素材占挂件底部 59.45% 高度，
...
var currentRole = { id: 'default', name: '大肥鱼', url: IMG_URL }
var roleList = []
function loadRoles() {
  try {
      ...
      fetch('/dsh-whale/role-delete.json', {
        ...
        .then(function (d) {
          if (d && d.ok && Array.isArray(d.roles)) {
            roleList = d.roles
            renderRolePanel()
            if (currentRole.id === id) applyRole('default', '大肥鱼', IMG_URL)
          }
        })
        .catch(function () {})
    } catch (err) {}
...
function setupHitTest(url) {
  try {
    ...
    probe.onerror = function () {
      // 图片加载失败：不能把整页当成大肥鱼命中区吞掉事件（会全页面点不动），
      // 标记失败，命中判定退回图像矩形区域。
      hitFailed = true
    }
...
// 右键大肥鱼唤出菜单(仅在开启「隐藏菜单按钮」时生效;菜单位置与按钮唤出一致)
...
// ===== 移动端触摸支持(v631) =====
// 大肥鱼是 pointer-events:none 的穿透层,手指真正按到的其实是下层页面元素(通常是可滚动区),
// 浏览器会把这次手势当成页面滚动 → 派发 pointercancel 掐断拖拽,表现为"手机上拖不动"。
// 这里只在"触摸起点命中大肥鱼"的这一次手势上阻止滚动(非 passive 监听),让 pointer 事件得以持续;
...
var touchDrag = null // 正在接管滚动的触摸(仅"起点命中大肥鱼"的那一次手势)
...
var touchEndedAt = 0 // 大肥鱼手势结束时刻(用于吞掉随之补发的原生 contextmenu)
...
function fireTouchLongPressMenu() {
  ...
  // 长按只用于唤出菜单:就地收尾当前拖拽(不移动大肥鱼),随后的抬手也不再触发 whaleClick
...
// 窗口尺寸变化时：自由位置的大肥鱼按相对边框锚点重算（保持离边距离，窗口恢复原状即回原位）；
// 贴边吸附的大肥鱼走 settle()（保持贴边）
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .then(function (d) {
    ...
    if (d && typeof d.usageMode === 'string') {
      usageMode = 'ledger' // 旧 token 配置自动视为大肥鱼记账
    }
