...
(function(){
  ...
  function setInteractive(v){if(!window.RikkaDaFeiYu||lastInteractive===v)return;lastInteractive=v;try{window.RikkaDaFeiYu.setInteractive(v)}catch(e){}}
  function reportRect(){try{var r=document.querySelector('.dshwv-root');if(!r||!window.RikkaDaFeiYu)return;var b=r.getBoundingClientRect(),next=[b.left,b.top,b.right,b.bottom,window.innerWidth,window.innerHeight];if(lastRect&&next.every(function(v,i){return Math.abs(v-lastRect[i])<.5}))return;lastRect=next;window.RikkaDaFeiYu.setWidgetRect.apply(window.RikkaDaFeiYu,next)}catch(e){}}
  function sync(){try{var open=!!document.querySelector('.dshwv-menu.dshwv-menu-open,.dshwv-root.dshwv-dragging');if(!open){var n=document.querySelectorAll('.dshwv-menu,.dshwv-bubmask,.dshwv-audiomask,.dshwv-confirmmask,.dshwv-cropmask,.dshwv-snapmask,.dshwv-resmask,.dshwv-usage-mask,.dshwv-qedit,.dshwv-usagepanel,.dshwv-rolelist,.dshwv-audiolist,.dshwv-custmenu,.dshwv-hintbox');for(var i=0;i<n.length;i++){var e=n[i],c=getComputedStyle(e),r=e.getBoundingClientRect();if(c.display!=='none'&&c.visibility!=='hidden'&&r.width>1&&r.height>1){open=true;break}}}setInteractive(open);reportRect()}catch(e){}}
  function schedule(){if(timer)return;timer=setTimeout(function(){timer=0;sync()},80)}
  function install(){if(document.querySelector('.dshwv-embedded-settings-style'))return;var st=document.createElement('style');st.className='dshwv-embedded-settings-style';st.textContent=[
  ...
  function wait(){var r=document.querySelector('.dshwv-root');if(!r){setTimeout(wait,120);return}install();sync();try{new MutationObserver(schedule).observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style','hidden']})}catch(e){}setInterval(sync,1000)}
  wait();
})();
