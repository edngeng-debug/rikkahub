...
/** Offline-artwork floating DeepSeek pet. */
class LocalDeepSeekPetService : Service() {
    ...
    private fun randomPhrase() = listOf(
        "欸！你戳我干嘛～", "轻一点啦……", "捏一下就跑？",
    ...
    private fun startRefreshing() {
        ...
        refreshJob = scope.launch {
            refreshBalance()
            while (isActive) { delay(60_000); refreshBalance() }
        }
    }
    ...
    private fun refreshBalance() {
        ...
        scope.launch(Dispatchers.IO) {
            ...
            launch(Dispatchers.Main) {
                result?.let { balanceText?.text = "$it credits"; balanceText?.isVisible = true }
            }
        }
    }
    ...
    companion object {
        ...
        fun start(context: Context, apiKey: String) {
            val intent = Intent(context, LocalDeepSeekPetService::class.java).apply {
                action = ACTION_START; putExtra(EXTRA_KEY, apiKey)
            }
